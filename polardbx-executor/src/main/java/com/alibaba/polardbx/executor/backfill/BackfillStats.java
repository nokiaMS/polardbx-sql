...
public class BackfillStats {
    ...
    /**
     * 此变量的功能是限制采样的行数，过多的采样行数会导致采样阶段的性能问题，过少的采样行数会导致分裂不均匀，无法分裂等问题。
      1. 采样行数过多时，采样阶段需要扫描更多的数据，增加了采样的时间和资源消耗。
      2. 采样行数过少时，可能无法准确地反映数据的分布情况，导致分裂不均匀或者无法分裂，从而影响后续的回填效率。
     默认值为20万行，这个值是根据实际的测试和经验得出的，既能保证采样的准确性，又能控制采样的性能开销。
      1. 当数据量较大时，可以通过调整这个值来控制采样的行数，从而平衡采样的准确性和性能。
      2. 当数据量较小时，可以适当增加这个值，以获得更准确的采样结果。
     */
    private long maxSampleRows = 200_000L;

    /**
     * 默认采样率为5%，当数据量较大时，可以通过调整采样率来控制采样的行数，从而平衡采样的准确性和性能。
      1. 采样率过高时，可能会导致采样的行数过多，增加了采样的时间和资源消耗。
      2. 采样率过低时，可能会导致采样的行数过少，无法准确地反映数据的分布情况，影响分裂的效果。
     */
