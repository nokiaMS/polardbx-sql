 ...
 */
public interface RelNode extends RelOptNode, RelHint, Cloneable, RelRelatedId {
  //~ Methods ----------------------------------------------------------------
  ...
  /**
   * 自带的explain函数还需要用作digest，每一次查询优化都会调用的，不适宜太长或者消耗性能比较多，所以为了展示用的执行计划单独加一个新的方法
   *
   * @param pw
   */
  void explainForDisplay(RelWriter pw);
